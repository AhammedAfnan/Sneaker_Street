...
const loadProductOverview = async (req, res, next) => {
  try {
    ...
    const pdtData = await Products.findById({ _id: id }).populate(
      "reviews.userId"
    );
    ...
    const discountPercentage = Math.floor(
      100 - (currPrice * 100) / pdtData.price
    );
...
const loadAddReview = async (req, res, next) => {
  try {
    ...
    product?.reviews.forEach(rvw => {
      if (rvw.userId._id == userId) {
        isReviewd = true
      }
    })

    if (isReviewd) return res.redirect(`/editReview/${productId}`)
...
const postAddReview = async (req, res, next) => {
  try {
    ...
    const totalRating = pdtData.reviews.reduce(
...
const loadEditReview = async (req, res, next) => {
  try {
    ...
    const reviewData = pdtData.reviews.find(
